...
const Map = ({
 idMap, indexMap
  ...
  const onClick_Map = (event, indexMap) => {
    replaceData2CompGallery("edit", "locationAddingMap", [indexMap]);
    replaceData2CompGallery("edit", "whichAdding", "Map");
...
const MapsReady = ({
  ...
  const onClick_Plus = (event, indexItem) => {
    replaceData2CompGallery("edit", "locationAddingMap", [indexItem]);
    replaceData2CompGallery("edit", "whichAdding", "Map");
  }
...
function mapStateToProps(state) { 
  return { 
    ...
    , triggerMap: state.comp_gallery.edit.triggerMap
  }; 
} 
